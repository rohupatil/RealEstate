...
export default function Profile() {
  ...
  const { currrentUser,loading,error } = useSelector((state) => state.user);
...
const handledelete= async ()=>{
  ...
  const handleSubmit=  async (e)=>{
    ...
    try {
      ...
      const res =await fetch(`/api/user/update/${currrentUser._id}`,
      {
        method: 'POST',
        headers:{
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input
          type="text"
          id="username"
          defaultValue={currrentUser.username}
        ...
        <input
          type="email"
          id="email"
          defaultValue={currrentUser.email}
          placeholder="email"
          className="border p-3 rounded-lg"
          onChange={handleChange}
        />
      ...
      <p className="text-green-700 mt-5" >{updateSuccess ? 'User Updaed successfully!' : ''}</p>
